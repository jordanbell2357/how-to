...
```
netcdf climate_summaries_ON {
dimensions:
    records = UNLIMITED;  // Number of rows in the CSV file
    str_len = 50;         // Maximum string length for string variables

variables:
    // Defining variables as per the CSV columns
    float Long(records);
        Long:units = "degrees_east";
        Long:long_name = "Longitude";
        Long:_FillValue = -9999.9;  // Fill value for missing data
    float Lat(records);
        Lat:units = "degrees_north";
        Lat:long_name = "Latitude";
        Lat:_FillValue = -9999.9;
    char Stn_Name(records, str_len);
        Stn_Name:long_name = "Station Name";
    int Clim_ID(records);
        Clim_ID:long_name = "Climate Identifier";
    char Prov_or_Ter(records, str_len);
        Prov_or_Ter:long_name = "Province or Territory";
    float Tm(records);
        Tm:units = "Celsius";
        Tm:long_name = "Mean Temperature";
        Tm:_FillValue = -9999.9;
    int DwTm(records);
        DwTm:long_name = "Days without Valid Mean Temperature";
        DwTm:_FillValue = -9999;
    float D(records);
        D:units = "Celsius";
        D:long_name = "Mean Temperature difference from Normal (1981-2010)";
        D:_FillValue = -9999.9;
    float Tx(records);
        Tx:units = "Celsius";
        Tx:long_name = "Highest Monthly Maximum Temperature";
        Tx:_FillValue = -9999.9;
    int DwTx(records);
        DwTx:long_name = "Days without Valid Maximum Temperature";
        DwTx:_FillValue = -9999;
    float Tn(records);
        Tn:units = "Celsius";
        Tn:long_name = "Lowest Monthly Minimum Temperature";
        Tn:_FillValue = -9999.9;
    int DwTn(records);
        DwTn:long_name = "Days without Valid Minimum Temperature";
        DwTn:_FillValue = -9999;
    float S(records);
        S:units = "cm";
        S:long_name = "Snowfall";
        S:_FillValue = -9999.9;
    int DwS(records);
        DwS:long_name = "Days without Valid Snowfall";
        DwS:_FillValue = -9999;
    float S_N(records);
        S_N:units = "percent";
        S_N:long_name = "Percent of Normal (1981-2010) Snowfall";
        S_N:_FillValue = -9999.9;
    float P(records);
        P:units = "mm";
        P:long_name = "Total Precipitation";
        P:_FillValue = -9999.9;
    int DwP(records);
        DwP:long_name = "Days without Valid Precipitation";
        DwP:_FillValue = -9999;
    float P_N(records);
        P_N:units = "percent";
        P_N:long_name = "Percent of Normal (1981-2010) Precipitation";
        P_N:_FillValue = -9999.9;
    float S_G(records);
        S_G:units = "cm";
        S_G:long_name = "Snow on the ground at the end of the month";
        S_G:_FillValue = -9999.9;
    int Pd(records);
        Pd:long_name = "Number of days with Precipitation 1.0 mm or more";
        Pd:_FillValue = -9999;
    float BS(records);
        BS:units = "hours";
        BS:long_name = "Bright Sunshine";
        BS:_FillValue = -9999.9;
    int DwBS(records);
        DwBS:long_name = "Days without Valid Bright Sunshine";
        DwBS:_FillValue = -9999;
    float BS_N(records);
        BS_N:units = "percent";
        BS_N:long_name = "Percent of Normal (1981-2010) Bright Sunshine";
        BS_N:_FillValue = -9999.9;
    float HDD(records);
        HDD:units = "Degree Days";
        HDD:long_name = "Degree Days below 18 °C";
        HDD:_FillValue = -9999.9;
    float CDD(records);
        CDD:units = "Degree Days";
        CDD:long_name = "Degree Days above 18 °C";
        CDD:_FillValue = -9999.9;

// Global Attributes
    :title = "Climate Summaries for Ontario";
    :source = "CSV data";
    :description = "Monthly climate summaries for stations in Ontario.";

// Data is not included here, it will be filled in with ncgen
}
```
